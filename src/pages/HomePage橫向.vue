            ...
            >
              ...
              <q-item-section>
                <q-item-label>
                  ...
                  {{ event.date }} ({{ getWeekday(event.date) }}) {{ event.time }} -
                  {{ event.title }}
                </q-item-label>
                <q-item-label caption> 剩 {{ event.daysLeft }} 天 </q-item-label>
              </q-item-section>
...
<script setup>
  ...
  function loadUpcomingEvents() {
    ...
    upcomingEvents.value = allEvents
      .filter(evt => {
        const evtDate = clearTime(new Date(evt.date))
        return evtDate >= today && evtDate <= sevenDaysLater
      })
      .map(evt => {
        const daysLeft = Math.ceil((clearTime(new Date(evt.date)) - today) / (1000 * 60 * 60 * 24))
        return { ...evt, daysLeft }
      })
  ...
  function getWeekday(dateStr) {
    ...
    if (isNaN(date)) return '' // 若日期格式錯誤回空字串
